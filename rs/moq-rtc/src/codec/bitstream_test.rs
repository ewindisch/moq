...
//! regressions in the contract the moq-rtc bridge depends on. They live in-crate
//! (rather than under `tests/`) because they exercise the crate-private `codec`
//! plumbing directly.
...
// then sets up an egress `Track` against the same broadcast and walks the
// catalog rendition through `Track::next()`. Verifies that the emitted
// payload is in the shape str0m's Frame API expects:
//
